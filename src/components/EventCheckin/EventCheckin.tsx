...
export default function EventCheckin() {
  ...
  const auth = useAuth();
  const { classId, scheduleId, scheduleDate, setCurrentEvent } =
    useSessionStore();
  ...
  const [selectedChildForSticker, setSelectedChildForSticker] =
    useState<any>(null);
    ...
    try {
      ...
      stickerDiv.innerHTML = `
        ...
        ">
          ...
          ">
            ...
            <div style="font-size: 10px; background: #f0f0f0; padding: 2px 6px; border-radius: 3px;">${
              child.age
            } anos</div>
          </div>
            ...
            ">
              <div style="font-size: 8px;">
                <strong>Autorização de imagem:</strong> ${
                  child.image_authorization ? "SIM" : "NÃO"
                }
              </div>
      ...
      if (navigator.share) {
        await navigator.share({
          files: [
            new File([blob], `etiqueta-${child.name}.png`, {
              type: "image/png",
            }),
          ],
      ...
      <S.Card>
        {showChildSelector && (
          <S.ChildSelectorContainer>
            <ChildSelector scheduleId={scheduleId} onCheckIn={handleCheckIn} />
            <Button
              type="button"
              color="secondary"
              size="sm"
                    ...
                    <S.MenuContainer>
                      <S.MenuButton
                        onClick={(e: React.MouseEvent) => {
                          e.stopPropagation();
                          setShowMenuDropdown(
                            showMenuDropdown === record.id ? null : record.id
                          );
                        }}
                      >
                        ⋯
                      ...
                      {showMenuDropdown === record.id && (
                        <S.MenuDropdown>
                          {record.status === "present" && (
                            <S.MenuItem
                              onClick={() => handleRelease(record.childId)}
                            >
                              🚪 Liberar
                            </S.MenuItem>
                          )}
                          <S.MenuItem
                            onClick={() =>
                              handleShowStickerPreview(record.child)
                            }
                          >
                            🏷️ Etiqueta
                          </S.MenuItem>
                          <S.MenuItem
                            onClick={() => handleWhatsApp(record.child)}
                          >
                            💬 WhatsApp
                          </S.MenuItem>
                          <S.MenuItem
                            onClick={() => handleShowChildDetails(record.child)}
                          >
                            👁️ Ver Detalhes
                          </S.MenuItem>
                        </S.MenuDropdown>
          ...
          <S.DetailsModalContent>
            ...
            <S.DetailsContent>
              ...
              <S.DetailRow>
                ...
                <S.DetailValue>
                  {selectedChildForDetails.age} anos
                </S.DetailValue>
              </S.DetailRow>
              ...
              <S.DetailRow>
                ...
                <S.DetailValue>
                  {selectedChildForDetails.guardian.name}
                </S.DetailValue>
              </S.DetailRow>
              ...
              <S.DetailRow>
                ...
                <S.DetailValue>
                  {selectedChildForDetails.guardian.phone}
                </S.DetailValue>
              </S.DetailRow>
            ...
            <S.StickerPreviewContainer>
              <S.StickerPreview>
                <S.StickerHeader>
                  ...
                  <S.StickerAge>
                    {selectedChildForSticker.age} anos
                  </S.StickerAge>
                </S.StickerHeader>
                ...
                <S.StickerContent>
                  <S.StickerInfo>
                    ...
                    {selectedChildForSticker.guardian.name}
                  </S.StickerInfo>
                  <S.StickerInfo>
                    <strong>Telefone:</strong>{" "}
                    {selectedChildForSticker.guardian.phone}
                  </S.StickerInfo>
                ...
                <S.StickerFooter>
                  <S.StickerAuth>
                    <strong>Autorização de imagem:</strong>{" "}
                    {selectedChildForSticker.image_authorization
                      ? "SIM"
                      : "NÃO"}
                  </S.StickerAuth>
                </S.StickerFooter>
